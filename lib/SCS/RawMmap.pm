...
sub new {
	...
	$self->{path}         = $ENV{SCSRAWPATH} || '/data/cruise/jcr/current/scs/Raw';
	$self->{xml_desc}     = undef;
	$self->{files}        = [ ];
	$self->{num_files}    = 0;
	$self->{current_file} = 0;
...
sub _convert_string {
	...
	if (defined($var->{spos}) && defined($var->{epos})) {
		$val = substr($rawrec, $var->{spos}, $var->{epos} - $var->{spos} + 1);
	} else {
		$val = $infs->[$var->{field}];
	}
...
sub _convert_number {
	...
	my $valstr = undef;
	# Check for start / endpos
	if (defined($var->{spos}) && defined($var->{epos})) {
		$valstr = substr($rawrec, $var->{spos}, $var->{epos} - $var->{spos} + 1);
	} else {
		$valstr = $infs->[$var->{field}];
	}

	$valstr = '' unless defined($valstr);

	my $val = $valstr;

	# Check for base to convert if necessary
	if ($var->{base} && ($var->{base} != 10)) {
		# Currently only works for integer conversion
		$val = cnv($valstr, $var->{base}, 10);
	} 

	# Check for measured - world conversion
	if (defined($var->{wmax}) && defined($var->{wmin}) && defined($var->{mmax}) & defined($var->{mmin})) {
		$val = $var->{wmin} + $val * (($var->{wmax} - $var->{wmin}) / ($var->{mmax} - $var->{mmin}));
	}

	return (looks_like_number($val) ? sprintf("%.6f", $val) : $val);
}
